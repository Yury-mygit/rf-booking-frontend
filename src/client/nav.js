...
// Bottom-nav клиента. 3 пункта: отели / отель / брони. active ∈
// {"hotels","hotel","bookings"}. Кнопка «Отель» ведёт на последний открытый
// отель (state.lastHotel); если отеля нет — на /client/hotels.
// Кнопки «Комнаты» / «Услуги» вынесены из nav на сам view отеля
// (`.hotel-quick-actions` над bottom-nav).
export function clientNavItems(active) {
  const h = getLastHotel();
  const slug = h ? (h.slug || h.id) : null;
  const hotelHref = slug ? `/client/hotel/${encodeURIComponent(slug)}` : "/client/hotels";
